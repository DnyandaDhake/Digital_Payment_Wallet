  ...
  try {
    ...
    for (const participant of participants) {
      ...
      const notification = new Notification({
        ...
        message: `${req.user.name} has requested ₹${participant.amount} for "${title}"`,
        relatedSplit: split._id
      });
    ...
    res.status(201).json({
      success: true,
      message: 'Money split and notifications sent successfully',
      split
    });
